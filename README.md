...
```env
# IRIS database connection (used by all services)
IRIS_HOST=iris
IRIS_NAMESPACE=USER
...
```bash
curl -s -X POST localhost:8000/service-requests \
...
curl -s -X POST localhost:8000/service-requests \
  ...
  -d '{
    ...
    "items": [
      { "exam_code": "ERR-LOINC", "exam_name": "Unknown LOINC test (simulator)" }
    ]
  }'
```
...
curl -s -X POST localhost:8000/service-requests \
  ...
  -d '{
    ...
    "notes": "Test 4 — VitaCare covenant expired, agent routes to vitacare",
    "items": [
